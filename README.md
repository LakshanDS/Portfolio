...
For automated CI/CD, you can use the `install.sh` script in your GitHub Actions workflow:
...
|----------|-------------|-----------|---------|
| `DATABASE_URL` | Database connection string (SQLite file path) | Yes | `file:./dev.db` |
| `SESSION_SECRET` | Secret key for session encryption (64-character random string) | Yes | - |
...
- **Database files**: `*.db`, `*.db-journal`, Prisma migration databases
- **Build outputs**: `.next/`, `dist/`, `build/`
- **Environment files**: `.env`, `.env.local`, `.env.*.local` (except `.env.example`)
- **IDE files**: `.vscode/`, `.idea/`, editor configs
- **Cache files**: `.turbo/`, `.cache/`, `.eslintcache`
- **Log files**: `*.log`, npm/yarn debug logs
- **OS files**: `.DS_Store`, `Thumbs.db`, etc.
- **Temporary files**: `*.tmp`, `*.temp`

The `.env.example` file is tracked as a template for setting up environment variables.

## Database Seeding

The application includes two seed files:

- **seed.ts**: Additive seeding - adds data without removing existing
- **seed-fresh.ts**: Fresh seeding - creates complete dataset (recommended for new installations)

To seed with fresh data:
```bash
npm run db:seed
```
...
- 5 Projects
- 8 Roadmap Items
- 1 User with TOTP secret for admin authentication
